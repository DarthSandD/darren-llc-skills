...
description: 'Use when running Windows-native commands from git-bash.'
---

# Windows Shell Interop

The terminal runs bash (git-bash/MSYS); Windows-native tools (powershell.exe, taskkill.exe, ffmpeg, python) need native conventions. MSYS path conversion is disabled, so get paths and quoting right.
