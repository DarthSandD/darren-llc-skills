---
name: android-apk-ship
description: Use when building, verifying, or handing over Android APKs.
version: 0.1.0
author: Darren Lieu, Hermes Agent
license: MIT
platforms: [windows]
metadata:
  hermes:
    tags: [Android, Gradle, APK, Verification, Delivery]
    related_skills: []
---

# Android APK Ship Skill

Build an Android repo with real Gradle, prove the APK is what you claim,
then hand it over with the report shape Darren expects. Covers the traps
that silently produce stale or crashing APKs.

## When to Use

- Any task ending in an installable APK (debug, release-signed, or both).
- User reports a delivered APK crashes or misbehaves.
- Don't use for: store listing copy, Play Console uploads, non-Android builds.
...
Lead with APK paths + sizes. Then: deleted vs rewritten, real-vs-stubbed
with the one in-memory exception named, left-out items with reasons.
No process replay, no tool-output dumps.
