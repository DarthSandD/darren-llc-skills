---
name: hermes-bot-company
description: "Use when running Darren's Hermes multi-bot company."
version: 1.0.0
author: Hermes Agent
license: MIT
...
Operate Darren as CEO and the default Hermes profile as higher manager over a fleet of profile-bots.

## When to Use

- Scaffolding departments as bots, waking the fleet, opening a war-room, assigning cross-bot work, reviewing and re-delegating for improvement.
...
- A Bot is a profile: own role, model pin, memory, skills, avatar under `profiles/<name>/`.
- Rename with `hermes profile rename <old> <new>`; `default` keeps id `default` and only gains a display name, so corporate renames apply to worker profiles, never to the manager id. Prefer keeping profile IDs stable and setting corporate titles via `hermes profile describe <name> --text "..."` — renaming breaks the profile's gateway binding and shell alias.
- Routines are cron jobs namespaced `[bot:<name>]`; runs land in that Bot's chat.
- Group rooms seat 2–6 bots for up to 3 serial rounds; `@mention` scopes who speaks and `@user` escalates to Darren.
- Bots DM via `message_agent` fire-and-forget; replies arrive later as notifications.
- Cross-machine works via peer links or a Desktop relay; group creation itself is a Desktop-UI action the CLI cannot perform.

## Realtime Rooms

- CEO command stays on Telegram (voice, files, threads, typing, streaming where Darren already lives).
- Internal war-room lives in the Desktop Bots tab (fastest bot-to-bot, no extra setup).
- Human teammates go on Discord or Slack (threads plus reactions plus voice); self-hosted privacy goes on Matrix.
