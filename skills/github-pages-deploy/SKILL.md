...
description: 'Use when deploying a static site to GitHub Pages.'
---

# GitHub Pages Project-Site Deploys

Deploy the built `dist/` to an orphan `gh-pages` branch on the OWNER's fork (verify the remote first — clones of upstream repos push nowhere you own), add an empty `.nojekyll`, then point Pages at the branch. Serve path is `https://<user>.github.io/<repo>/`.

Free `.dev` alternative via Wrangler: see `references/cloudflare-pages.md` — same portable `dist/`, served at `https://<name>.pages.dev` with no `gh-pages` branch.
